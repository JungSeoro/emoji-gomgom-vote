...
현재 PNG 29개를 참고 이미지의 위에서 아래 순서로 등록했습니다. 같은 행에 있던 6개 그룹은 세트로 구성했고 나머지 13개는 단독 후보로 구성했습니다. `Frame 12.png`와 `Frame 435.png`는 거의 동일하지만 참고 이미지에서 서로 다른 행에 있으므로 각각 별도 후보로 유지합니다.

고해상도 PNG 원본은 `asset`에 보존하고, 화면에서는 `asset/optimized`의 긴 변 640px 투명 WebP를 사용합니다. 원본을 교체하거나 추가한 뒤 실행 중인 컨테이너에서 다음 명령으로 파생본을 다시 생성합니다.
