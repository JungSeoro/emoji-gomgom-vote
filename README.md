...
관리자 화면에서는 입력한 문자열이 포함된 닉네임의 제출 결과와 피드백을 검색할 수 있습니다. 제출 항목은 최대 100개까지 여러 개 선택해 후보별 득표와 피드백 집계에서 제외하거나 다시 포함할 수 있으며, 제외된 항목도 목록에는 그대로 표시됩니다. 닉네임 검색과 페이지를 변경해도 유효한 제외 선택은 유지되고, `Clear` 버튼으로 전체 제외를 초기화합니다. 비밀번호 원문과 Service role key는 프런트 코드에 포함하지 않습니다. bcrypt 제한에 맞춰 비밀번호는 UTF-8 기준 72바이트 이하로 설정합니다. 로그인은 IP별 5회 실패 시 15분 동안 제한되며, 접근 세션은 최대 2시간·미사용 30분 후 만료됩니다. 세션 토큰은 현재 탭의 `sessionStorage`에만 저장됩니다.
